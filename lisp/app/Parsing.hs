...
-- What on its face looks like it ought to be a very complicated combinator is in 
-- fact very strait forward. We create a new parser that acts just like p but 
-- will call the “empty error (eerr) continuation even if p consumed input. 
    --  The state s is actually captured in <|> rather then try.
